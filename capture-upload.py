...
def create_workers(cameras, q):
    """
    Creates thread workers from a dict of port:serialnumber strings.
    :param cameras:
    :return:
    """
    return [(GphotoCamera(q, sn, port), Uploader(q, sn)) for port, sn in cameras.items()]

def default_identifier():
    if name:
        self.serialnumber = SysUtil.get_identifier_from_name(name)
    else:
