            ...
            if (self.time2seconds(tn) % self.interval < self.accuracy) and (tn.time() > self.timestartfrom) and (
                ...
                try:
                    ...
                    if self.config.has_section("picam_size"):
                        os.system("/opt/vc/bin/raspistill -w " + self.config["picam_size"]["width"] + " -h " + self.config["picam_size"]["height"] + " --nopreview -o " + image_file)
                    else:
    ...
    def set_metadata_on_server(self, list_of_uploads):
        ...
        try:
            ...
            try:
                ...
                jsondata["capture_limits"] = self.config['timelapse']['starttime'] + " - " + self.config['timelapse']['stoptime']
...
def create_workers(cameras):
    ...
    for port, serialnumber in list(cameras.items()):
        camthreads.append(
            Camera(os.path.join("configs_byserial", serialnumber + ".ini"), camera_port=port, serialnumber=serialnumber,
                   name=serialnumber))
...
if __name__ == "__main__":
    ...
    try:
        ...
        while cameras is None and tries < 10:
